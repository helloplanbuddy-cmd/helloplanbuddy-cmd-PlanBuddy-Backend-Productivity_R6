...
### COMPLERE FULL BACKEND 100% REAL SYSTEM PRODYUCITON .

- Repository https://github.com/helloplanbuddy-cmd/PlanBuddy-Backend-Productivity_R2
...
```sh
git clone https://github.com/helloplanbuddy-cmd/PlanBuddy-Backend-Productivity_R2
cd planbuddy-backend
npm install
npm run serve
```
...
The planbuddy-backend boilerplate is using the below vscode extensions.
...
Use the HubAI CLI to generate applications inside this workspace. Run commands from the workspace root.

```sh
# Create a React app
hubi create:app app-web -t react
...
# Create a Subgraph service (Apollo Federation)
hubi create:app app-users -t subgraph

# Create an Apollo Gateway
hubi create:app app-gateway -t gateway

# Create a Cypress E2E testing project
hubi create:app app-web-e2e -t cypress
```

Notes:
- Some generators will ask additional questions (ports, endpoints, libs).
- If you accidentally run from a folder without a workspace, the CLI will prompt to create a workspace first and then continue.
